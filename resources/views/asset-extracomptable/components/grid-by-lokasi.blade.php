<template id="grid-extracomptable-by-lokasi">
    ...
    <table class="table table-bordered table-hover no-margin">
      ...
      <tbody>
        <tr v-for="(asset, i) in assets">
          <td>@{{ offset + i }}</td>
          <td>@{{ asset.gedung }}</td>
          <td class="text-center">@{{ asset.lantai }}</td>
          <td>@{{ asset.ruang }}</td>
...
@script('component-grid-extracomptable-by-lokasi')
  @parent
  <script>
  Vue.component('grid-extracomptable-by-lokasi', {
    template: '#grid-extracomptable-by-lokasi',
    props: {
      urlFetch: {
        type: String,
        required: true
