...
@section('content')
  @component('components.panel-form')
    @slot('title')
      <i class="fa fa-plus"></i> Edit Request Inventory
    @endslot

    <form action="{{ route('request-inventory::post-edit', [$request->id]) }}" method="POST" class="form-horizontal">
        ...
        <table style="margin-top: 10px" class="table table-striped table-hover table-bordered">
          ...
          <tbody class="vhide hidden">
            ...
            <tr v-for="item, index in list_items">
              ...
              <td>@{{ item.note }}</td>
              <td>
                <input type="hidden" :name="'items['+index+'][id_kategori]'" :value="item.kategori.id"/>
      ...
      <div class="form-group">
        <div class="col-md-push-2 col-md-10">
          <a href="{{ route('AdminRequestInventoryControllerGetIndex') }}" class="btn btn-default"><i class="fa fa-chevron-left"></i> Batal</a>
          &nbsp;
  ...
  @endcomponent
  @include('request-inventory.modal-pick-item')
@endsection
