...
use App\Traits\GetMasterOptions;
	...
	class AdminAdjustmentExtracomptableController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "adjustment_extracomptable";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Kode Adjustment","name"=>"kd_adjustment"];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Kode Adjustment','name'=>'kd_adjustment','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"Kd Adjustment","name"=>"kd_adjustment","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
        ...
        public function getAdd()
        {
            ...
            $data['kd_adjustment'] = AdjustmentExtracomptable::generateKodeAdjustment();
            $this->cbLoader();
            return view('adjustment-extracomptable.form-add', $data);
        }
        ...
        public function getEdit($id)
        {
            ...
            $data['options_gedung'] = $this->getOptionsGedung();
            $data['options_lantai'] = $this->getOptionsLantai($adjustment->id_gedung);
            $data['options_ruang'] = $this->getOptionsRuang($adjustment->id_gedung, $adjustment->lantai);
            $data['list_items'] = $this->getListItems($adjustment);
            $this->cbLoader();
            return view('adjustment-extracomptable.form-edit', $data);
        }

        protected function getListItems(AdjustmentExtracomptable $adjustment)
        {
            return $adjustment->items()->orderBy('id', 'asc')->get()->map(function($item) {
                return [
                    'jenis' => $item->subjenis->jenis->toArray(),
                    'subjenis' => $item->subjenis->toArray(),
