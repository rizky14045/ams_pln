...
use App\Traits\GetMasterOptions;
	...
	class AdminRequestExtracomptableController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "request_extracomptable";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Kode Request","name"=>"kd_request"];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Kode Request','name'=>'kd_request','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"Kd Request","name"=>"kd_request","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
        ...
        public function getAdd()
        {
            ...
            $data['kd_request'] = RequestExtracomptable::generateKodeRequest();
            $this->cbLoader();
            return view('request-extracomptable.form-add', $data);
        }

        public function getEdit($id)
        {
            $request = RequestExtracomptable::findOrFail($id);
            $data['request'] = $request;
            $data['options_gedung'] = $this->getOptionsGedung();
            $data['options_lantai'] = $this->getOptionsLantai($request->id_gedung);
            $data['options_ruang'] = $this->getOptionsRuang($request->id_gedung, $request->lantai);
            $data['list_items'] = $this->getListItems($request);
            $this->cbLoader();
            return view('request-extracomptable.form-edit', $data);
        }

        protected function getListItems(RequestExtracomptable $request)
        {
            return $request->items()->orderBy('id', 'asc')->get()->map(function($item) {
                return [
                    'jenis' => $item->jenis->toArray(),
