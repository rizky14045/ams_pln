...
class CheckoutInventoryController extends Controller
{
    ...
    public function postCreate(Request $req)
    {
        ...
        if (!$saved) {
            return redirect()->route('AdminCheckoutInventoryControllerGetAdd')
                ->with('error', 'Something went wrong while saving data.');
        }

        return redirect()->route('AdminCheckoutInventoryControllerGetIndex');
    }
    ...
    public function postEdit(Request $req, $id)
    {
        ...
        $saved = $this->saveCheckout($req, $checkout);
        if (!$saved) {
            return redirect()->route('AdminCheckoutInventoryControllerGetEdit')
                ->with('error', 'Something went wrong while saving data.');
        }
    ...
    public function postApprove(Request $req, $id)
    {
        ...
        return redirect()->route('AdminCheckoutInventoryControllerGetIndex')->with([
