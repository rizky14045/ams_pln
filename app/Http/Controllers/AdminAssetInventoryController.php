...
use App\Traits\GetMasterOptions;
    ...
    class AdminAssetInventoryController extends \crocodicstudio\crudbooster\controllers\CBController {
        ...
        public function cbInit() {
            ...
            $this->col[] = ["label"=>"Kategori","name"=>"id_kategori","join"=>"kategori_inventory,id"];
            $this->col[] = ["label"=>"Nama Asset","name"=>"nama_asset"];
            $this->col[] = ["label"=>"Jumlah","name"=>"jumlah"];
            $this->col[] = ["label"=>"Jumlah Minimum","name"=>"jumlah_minimum"];
            $this->col[] = [
                "label" => "QRCode",
                "name" => "id",
                "sorting" => false,
                ...
                "callback" => function($row) {
                    $asset = AssetInventory::find($row->id);
            ...
            $this->form[] = ['label'=>'Kategori','name'=>'id_kategori','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'kategori_inventory,id'];
            ...
            $this->form[] = ['label'=>'Jumlah Minimum','name'=>'jumlah_minimum','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
            ...
            //$this->form[] = ["label"=>"Jumlah Minimum","name"=>"jumlah_minimum","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
        ...
        */
        public function hook_query_index(&$query) {
            ...
            if (!$listAksesRuang) {
                $query->where('asset_inventory.id', 0);
            } else {
                $query->whereIn('asset_inventory.id_ruang', $listAksesRuang);
            }

            $query->whereNotIn('asset_inventory.id_ruang', [7]);
        }
        ...
        */
        ...
        public function getDetail($id)
        {
            ...
            return view('asset-inventory/page-detail', $data);
        }

        public function getAdd()
        {
            $id_gedung = session('id_gedung') ?: old('id_gedung');
            $lantai = session('lantai') ?: old('lantai');
            $id_kategori = session('id_kategori') ?: old('id_kategori');

            $data['page_title'] = "Add Asset Inventory";
            $data['options_gedung'] = $this->getOptionsGedung();
            $data['options_kategori'] = $this->getOptionsKategori();
            if ($id_gedung) {
            ...
            $this->cbLoader();
            return view('asset-inventory.form-add', $data);
        }
        ...
        public function getEdit($id)
        {
            ...
            $data['page_title'] = "Edit Asset Inventory";
            $data['asset'] = $asset;
            $data['options_gedung'] = $this->getOptionsGedung();
            $data['options_kategori'] = $this->getOptionsKategori();
            ...
            $this->cbLoader();
            return view('asset-inventory.form-edit', $data);
        }
