...
use App\Traits\GetMasterOptions;
	...
	class AdminPemeliharaanExtracomptableController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "pemeliharaan_extracomptable";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Kode Pemeliharaan","name"=>"kd_pemeliharaan"];
			$this->col[] = ["label"=>"Nik Karyawan","name"=>"nik_karyawan"];
			$this->col[] = ["label"=>"Jenis Pemeliharaan","name"=>"id_jenis_pemeliharaan"];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Kode Pemeliharaan','name'=>'kd_pemeliharaan','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Nik Karyawan','name'=>'nik_karyawan','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Jenis Pemeliharaan','name'=>'id_jenis_pemeliharaan','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"Kd Pemeliharaan","name"=>"kd_pemeliharaan","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Nik Karyawan","name"=>"nik_karyawan","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Jenis Pemeliharaan","name"=>"id_jenis_pemeliharaan","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
        ...
        public function getAdd()
        {
            ...
            $data['options_id_jenis_pemeliharaan'] = $this->getOptionsJenisPemeliharaanExtracomptable();
            ...
            $data['kd_pemeliharaan'] = PemeliharaanExtracomptable::generateKodePemeliharaan();
            $this->cbLoader();
            return view('pemeliharaan-extracomptable.form-add', $data);
        }
        ...
        public function getEdit($id)
        {
            ...
            $data['options_karyawan'] = $this->getOptionsKaryawan();
            $data['options_gedung'] = $this->getOptionsGedung();
            $data['options_lantai'] = $this->getOptionsLantai($pemeliharaan->id_gedung);
            $data['options_ruang'] = $this->getOptionsRuang($pemeliharaan->id_gedung, $pemeliharaan->lantai);
            $data['list_items'] = $this->getListItems($pemeliharaan);
            $this->cbLoader();
            return view('pemeliharaan-extracomptable.form-edit', $data);
        }

        protected function getListItems(PemeliharaanExtracomptable $pemeliharaan)
        {
            return $pemeliharaan->items()->with(['asset'])->orderBy('id', 'asc')->get()->map(function($item) {
                $asset = $item->asset;
                return [
                    'id_asset' => $asset->id,
                    'kd_asset' => $asset->kd_asset,
                    'nama_asset' => $asset->nama_asset,
                    'jenis' => ($asset->jenis)? $asset->jenis->toArray() : null,
                    'subjenis' => ($asset->subjenis)? $asset->subjenis->toArray() : null,
                ];
            })->toArray();
        }
